...
import asyncio
import asyncpg
from config6 import dbConfig, matchlistParams, riotConfig
import json
import os
import ssl
...
def main() -> None:
    ...
    try:
        ...
        if os.path.isfile(r'.\MatchWIP\progress6.json') and os.access(r'.\MatchWIP\progress6.json', os.R_OK):
            with open(r'.\MatchWIP\progress6.json') as f:
                data = json.load(f)
        ...
        if not os.path.exists('MatchWIP'):
            os.mkdir('MatchWIP')
        with open(r'.\MatchWIP\progress6.json', 'w') as outputFile:
