...
import asyncio
import asyncpg
from config2 import dbConfig, matchlistParams, riotConfig
import json
import os
import ssl
...
def main() -> None:
    ...
    try:
        ...
        if os.path.isfile(r'.\MatchWIP\progress2.json') and os.access(r'.\MatchWIP\progress2.json', os.R_OK):
            with open(r'.\MatchWIP\progress2.json') as f:
                data = json.load(f)
        ...
        if not os.path.exists('MatchWIP'):
            os.mkdir('MatchWIP')
        with open(r'.\MatchWIP\progress2.json', 'w') as outputFile:
